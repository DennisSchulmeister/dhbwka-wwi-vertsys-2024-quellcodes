 ...
 * @param {string} staticDir Verzeichnis mit statischen Webinhalten
 * @param {Function[]} controllers Liste von Funktionen, die der Express App URL-Routen hinzufügen
 */
export function startServer({asciiArt, staticDir, controllers} = {}) {
    ...
    const server = app.listen(config.port, config.host, () => {
        logger.info(`Server läuft auf ${config.host}:${config.port}`);
    });
